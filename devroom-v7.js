...
async function api(path,body){
  ...
  try{j=raw?JSON.parse(raw):{}}catch{j={error:raw||path+' returned a non-JSON response'}}
...
async function loadQaBridge(){
  try{
    const r=await fetch('/api/oidc-status',{cache:'no-store'});const raw=await r.text();let j={};
